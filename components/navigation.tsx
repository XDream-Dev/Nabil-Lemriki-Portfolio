...
import { Home, Sparkles, User, Mail, Search, Sun, Moon } from "lucide-react"
...
const navItems = [
	...
	{ href: "/projects", icon: Sparkles, label: "Projects", sectionId: "projects" },
...
export function Navigation() {
	...
	const handleSectionClick = (sectionId: string, e: React.MouseEvent, href: string) => {
		if (sectionId === "hero") {
			if (pathname === "/") {
				e.preventDefault()
				window.scrollTo({ top: 0, behavior: "smooth" })
			}
			// If not on home, let Link handle navigation to home
		}
	}
		...
		<>
			...
			>
				...
				<button
					...
					title={`Switch to ${theme === "light" ? "dark" : "light"} mode`}
				>
					{theme === "light" ? <Moon className="w-5 h-5 mb-0.5" /> : <Sun className="w-5 h-5 mb-0.5" />}
					<span className="text-[10px] leading-none">{theme === "light" ? "Dark" : "Light"}</span>
				</button>
			</nav>
